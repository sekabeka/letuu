...
async def main():
    async with async_playwright() as play:
        ...
        pages = [await context.new_page() for _ in range (1, 25)]
        ...
        with pd.ExcelWriter('letu_result.xlsx', mode='w', engine='xlsxwriter', engine_kwargs={'options': {'strings_to_urls': False}}) as writer:
            for name, df in list(table.items()):
                ...
                while urls:
                    ...
                    for page in pages:
                        if urls:
                            task = asyncio.create_task(js(page, urls.pop(), prefix=prefixs.pop(), article=articles.pop()))
                            tasks.append(task)
...
async def js(page, url, **kwargs):
    while True:
        try:
            data = await (await page.goto(url)).json()
        except Exception:
            continue
        else:
            ...
            while True:
                try:
                    new_data = await (await page.goto(new_link)).json()
                except Exception:
                    continue
                else:
                    return parse(
