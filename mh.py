...
async def main():
    async with async_playwright() as play:
        ...
        with pd.ExcelWriter('products2.xlsx', mode='w', engine='xlsxwriter', engine_kwargs={'options': {'strings_to_urls': False}}) as writer:
            for name, df in list(table.items()):
                ...
                if name == 'Ссылка':
                    ...
                    for url, prefix, page in zip(urls, prefixs, pages):
                        tmp_result = await Links(page=page, url=url, prefix=prefix)
                        result += [i for i in tmp_result if i != None]
